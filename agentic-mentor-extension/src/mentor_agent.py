...
from model_config import OLLAMA_HOST, MODEL as OLLAMA_MODEL

# REPO_ROOT = Path(__file__).resolve().parent.parent
REPO_ROOT=Path(sys.argv[1])
AGENT_DIR = REPO_ROOT / "mentor_chat_files"
...
def render_questions(questions: list[dict]) -> str:
    ...
    blocks = []
    for number, question in enumerate(questions, 1):
        # Two spaces at end of line (or standard spacing) prevents collapsing
...
def build_agent(feature_dir: Path, phase_state: dict, model: str) -> Agent:
...
async def run_mentoring(session_id: str = "default_session", model: str = OLLAMA_MODEL) -> None:
    ...
    feature_dir = REPO_ROOT / "specs" / "spec"
    ...
    agent = build_agent(feature_dir, phase_state,model)
...
async def main():
    ...
    model = sys.argv[3] if len(sys.argv) > 3 else OLLAMA_MODEL
    await run_mentoring(session_id,model)
