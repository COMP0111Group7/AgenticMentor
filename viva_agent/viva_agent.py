...
from model_config import OLLAMA_HOST as ollama_host, VIVA_MODEL as model
from config import chat_history_dir, system_prompt_file, summary_file, spec_file, max_questions
...
file_history = FileHistoryProvider(storage_path=chat_history_dir)
...
spec_path = sys.argv[2] if len(sys.argv) > 2 else spec_file
...
def save_transcript(transcript, out_path="viva_transcript.md"):
...
async def main():
    ...
    for number, item in enumerate(questions, 1):
        print(f"Question {number}/{len(questions)}: {item['question']}\n")
