...
from model_config import OLLAMA_HOST, INGESTION_MODEL
...
# Ollama defaults to a tiny context window (2048-4096 tokens) unless a caller passes
# num_ctx explicitly -- it does NOT scale to whatever the loaded model actually supports.
# This prompt bundles instructions + loaded speckit skill content + the full student spec
# (and later, generated spec/plan/tasks), which easily exceeds that default and gets
# silently truncated, producing empty/garbled model output. Same fix already applied in
# mentoring_agent_v2/mentor_agent.py's NUM_CTX -- was missing here.
NUM_CTX = 32768
...
def build_agent(feature_name: str, model: str) -> Agent:
    # This session may only write inside its own feature directory (plus the one shared
    # bookkeeping file the speckit-specify skill also touches). Without this, the model is
    # free to invent its own feature-directory name if it drifts from the skill's resolved
    # SPECIFY_FEATURE_DIRECTORY (observed once: it wrote a whole spec/plan/tasks pipeline
    # under specs/sample_spec/ instead of the intended specs/<feature_name>/).
    feature_dir = (REPO_ROOT / "specs" / feature_name).resolve()
    feature_json_path = (REPO_ROOT / ".specify" / "feature.json").resolve()

    async def write_file(path: str, content: str) -> str:
        target = (REPO_ROOT / path).resolve()
        if REPO_ROOT not in target.parents and target != REPO_ROOT:
            return "ERROR: path escapes repo root"
        if target != feature_json_path and feature_dir not in target.parents and target != feature_dir:
            return (
                f"ERROR: this session is scoped to 'specs/{feature_name}/' -- write_file may "
                f"only write there (or to .specify/feature.json), not '{path}'. Do not create "
                "a new feature directory or short name; use the one already assigned."
            )
    ...
    feature_dir_rel = f"specs/{feature_name}"
    ...
    instructions = (
        ...
        f"SPECIFY_FEATURE_DIRECTORY is already resolved to '{feature_dir_rel}' for this "
        "session -- this satisfies the speckit-specify skill's own resolution order (\"if "
        "the user explicitly provided SPECIFY_FEATURE_DIRECTORY, use it as-is\"). Do NOT "
        "generate your own short name or a numbered/timestamped directory under specs/ -- "
        f"use exactly '{feature_dir_rel}' for every file this session writes (spec.md, "
        "plan.md, tasks.md, checklists/, learning_objectives.md, etc). write_file is "
        f"restricted to this directory (plus .specify/feature.json) and will reject "
        "anything else.\n\n"
        "CLARIFY PHASE RULES:\n"
...
async def run_turn(agent: Agent, prompt: str, session):
    ...
    return await agent.run(prompt, session=session, options={"num_ctx": NUM_CTX})
...
async def run_clarify_loop(agent: Agent, session, spec_path: Path) -> None:
    ...
    result = await run_turn(agent, prompt, session)
    while CLARIFY_DONE_TOKEN not in result.text:
        print("\n--- Ingestion Agent needs clarification ---")
        print(result.text)
        # TODO: replace input() with however our solution collects the
        # student's answer (chat UI, website, etc.)
        student_answer = input("\nYour answer: ")
        if CLARIFY_DONE_TOKEN in student_answer:
            # Student manually forcing the move to speckit.plan
            break
        result = await run_turn(agent, student_answer, session)

async def run_ingestion(spec_path_str: str, context_path_str: str, model: str = INGESTION_MODEL) -> None:
    ...
    agent = build_agent(feature_name, model)
    ...
    print("=== RUNNING PLAN ===")
    result = await run_turn(agent, plan_prompt, session)
    print("=== PLAN OUTPUT ===")
    print(result.text)
    ...
    print("=== RUNNING TASKS ===")
    result = await run_turn(agent, tasks_prompt, session)
    print("=== TASKS OUTPUT ===")
    print(result.text)
    ...
    for attempt in range(1, MAX_TASKS_FORMAT_RETRIES + 1):
        ...
        result = await run_turn(agent, fix_prompt, session)
    ...
    result = await run_turn(agent, synthesis_prompt, session)
